...
export function generateBudgetAlertInsights(input: InsightEngineInput): FinancialInsight[] {
  ...
  for (const b of budgetUsage) {
    if (b.isExceeded) {
      ...
      insights.push({
        ...
        severity: 'danger',
        title: 'Przekroczono',
        description: `${b.categoryName}: +${formatAmountForInsight(over, currency)} ponad limit.`,
      ...
      insights.push({
        ...
        severity: 'warning',
        title: 'Blisko limitu',
        description: `${b.categoryName}: ${Math.round(b.usagePercent)}% budżetu.${daysText}`.trim(),
        relatedCategoryId: b.categoryId,
        relatedBudgetId: b.categoryId,
  ...
  if (allOk && budgetUsage.length > 0) {
    insights.push({
      ...
      severity: 'positive',
      title: 'Budżety OK',
      description: 'Wszystkie w normie.',
      ctaLabel: 'Zobacz budżety',
      ctaAction: 'budgets',
    });
  }
